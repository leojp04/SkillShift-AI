...
import type { SimpleUser } from "../contexts/AuthContext";
...
const NavLinks = ({ usuario, logout, theme, toggleTheme, isMobile = false }: NavLinksProps) => (
  <>
    ...
    <button
      ...
      className={`${base} bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 ${isMobile ? "" : "ml-2"}`}
    >
      {theme === "light" ? "🌙" : "☀️"}
    </button>
  </>
      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between px-4 py-3 gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <NavLinks usuario={usuario} logout={logout} theme={theme} toggleTheme={toggleTheme} />
          </div>
        </div>
      </div>
      {open && (
        <div className="sm:hidden border-t border-slate-200 dark:border-slate-700 px-4 pb-3 flex flex-col gap-2">
          <NavLinks usuario={usuario} logout={logout} theme={theme} toggleTheme={toggleTheme} isMobile />
        </div>
      )}
    </nav>
  );
